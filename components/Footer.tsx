...
import { Users, Mail, Phone, MapPin, Facebook, Instagram, Youtube, Heart } from 'lucide-react'
...
const Footer = () => {
  ...
  return (
    <footer className="bg-gradient-to-br from-gray-900 via-primary-900 to-earth-900 text-white">
              ...
              {footerLinks.academy.map((link) => (
                <li key={link.name}>
                  <motion.a
                    href={link.href}
                    whileHover={{ x: 5 }}
                    className="text-orange-200 hover:text-orange-300 transition-colors duration-300"
                  >
                    {link.name}
                  </motion.a>
                </li>
          ...
          >
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-orange-200">+51 987 654 321</span>
              </div>
              <div className="flex items-center space-x-3">
                <Mail className="w-4 h-4 text-primary-400 flex-shrink-0" />
                <span className="text-orange-200">info@academiaindigena.com</span>
              </div>
          ...
          <div className="text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg bg-white/20 border border-white/30 text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-500"
              />
              <motion.button
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <div className="flex items-center space-x-2 text-orange-200">
              <span>© {currentYear} Academia de Modelaje Indígena.</span>
              <span>Todos los derechos reservados.</span>
            </div>
            <div className="flex items-center space-x-1 text-orange-200">
              <span>Hecho con</span>
              <Heart className="w-4 h-4 text-red-400 fill-current" />
              <span>para celebrar la diversidad cultural</span>
            </div>
          </div>
        </motion.div>
      </div>
